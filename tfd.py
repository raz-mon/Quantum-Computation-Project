...
def infinite_temp(framework):
    ...
    qc.rxx(np.pi, qreg[0], qreg[3])
    qc.rxx(np.pi, qreg[1], qreg[4])
    qc.rxx(np.pi, qreg[2], qreg[5])
    qc.barrier()
    for n in range(6):
        qc.z(qreg[n])
...
def zero_temp(framework):
    ...
    qc = QuantumCircuit(qreg, creg)
    # Prepare system in tfd state.
    qc.rxx(np.pi, qreg[0], qreg[1])
    qc.rxx(np.pi, qreg[1], qreg[2])
    qc.rxx(np.pi, qreg[3], qreg[4])
    qc.rxx(np.pi, qreg[4], qreg[5])
    qc.barrier()
    for n in range(6):
        qc.z(qreg[n])
